...
@synthesize fscontroller = _fscontroller;
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self)
    {
        ...
        //add touch detector view
        TouchDetectorView *touchDetectorView = [[TouchDetectorView alloc] initWithFrame:CGRectMake(0, 0, 100, 80)];
        touchDetectorView.backgroundColor = [UIColor clearColor];
        touchDetectorView.theDelegate = self;
        
        [self.contentView addSubview:touchDetectorView];
...
-(void)TouchDetectorViewDidReceiveTouchupInsideEvent:(TouchDetectorView *)view;
{
    ...
    //NSLog(@"tap within view");
    NSIndexPath *indexPath = [_tabbar indexPathForCell:self];
    
    [_fscontroller selectRowAtIndexPath:indexPath withTableView:_tabbar];
}
